...
class TrainTestDataLoader:
    ...
    def Go(self, loader):
        ...
        kFeatures  = Var(lambda tables: pd.concat([v(tables) for v in self._feature_vars], axis=1))
    ...
    def PrepareTrainingAndTestData(self,
                                   ntrain_signal,
                                   ntrain_bkgd,
                                   ntest_signal,
                                   ntest_bkgd):
        ...
        self._train_signal, self._test_signal = train_test_split(self._x[self._y == 1],
                                                                 test_size  = ntest_signal,
                                                                 train_size = ntrain_signal)
        self._train_bkgd, self._test_bkgd = train_test_split(self._x[self._y == 0],
                                                             test_size  = ntest_signal,
                                                             train_size = ntrain_signal)
