...
class LocalTemplateFit:
    ...
    def release_template(self, template_idx):
        if np.isnan(self._param_mask.sum(axis=1)[template_idx]):
            self._param_mask = np.insert(
                self._param_mask, template_idx, np.zeros(self.T.shape[0]), axis=1
            )
    ...
    def minimize(self, data, *args, **kwargs):
        ...
        results = scipy.optimize.dual_annealing(
            self,
            bounds,
            *args,
            local_search_options={"method": "Newton-CG", "jac": self._jacobian},
            **kwargs,
        )
        end_time = time.perf_counter()

        results.x = self._to_user_coords(results.x)
        results.update({"k(V)": self.kV, "time": end_time - start_time})
        return results
    ...
    def _jacobian(self, params):
        u = self.U(params)
        return -1 * self._to_optimizer_coords(
            np.dot(self.T, np.dot(self.Vinv, (self.X - u).T))
            + np.dot(self.X - u, np.dot(self.Vinv, self.T.T))
        )
    ...
    def _to_user_coords(self, optimizer_params):
        return np.nan_to_num(
            np.dot(self._param_mask, optimizer_params)
        ) + np.nan_to_num(self._fixed_params)
...
class GlobalTemplateFit(LocalTemplateFit):
    def __init__(self, cov, templates, decorrelate=False):
        ...
        if np.prod(self.T.shape[1:]) != self.V.shape[0]:
            raise TypeError(
                f"Template shape ({self.T.shape}) not compatible with covariance matrix ({self.V.shape})"
            )
    ...
    def U(self, params):
        ...
        return (
            np.dot(np.diag(self._to_user_coords(params)), flat_T)
            .reshape((self.T.shape[0], np.prod(self.T.shape[1:])))
            .sum(axis=0)
        )
