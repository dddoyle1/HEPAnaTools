...
def fake_templates(nbins, nsignal=100, nbkgd0=400, nbkgd1=200, seed=None, shape=None):
    np.random.seed(seed)
    ...
    b1 = np.random.normal(0.5, 0.25, nbkgd1)
    ...
    if multi_dim:
        signal = np.array(
            [
                np.histogram(hs, nbins, range=(0, 1))
                for hs in s.reshape(np.prod(s.shape[:-1]), s.shape[-1])
            ]
        )
        bkgd0 = np.array(
            [
                np.histogram(hb0, nbins, range=(0, 1))
                for hb0 in b0.reshape(np.prod(b0.shape[:-1]), b0.shape[-1])
            ]
        )
        bkgd1 = np.array(
            [
                np.histogram(hb1, nbins, range=(0, 1))
                for hb1 in b1.reshape(np.prod(b1.shape[:-1]), b1.shape[-1])
            ]
        )
...
def plot_templates(
    ...
    if scales and len(scales) != len(templates):
        print(
            f"Scales ({scales.shape}) not compatible with templates ({templates.shape})"
        )
        exit
    if scales is None:
        scales = np.ones(len(templates))

    for temp, scale, label in zip(templates, scales, labels):
        plt.hist(
            bins[:-1],
            weights=np.dot(temp, scale),
            bins=bins,
            label=label,
            histtype="stepfilled",
            alpha=0.7,
...
def plot_mat(cov, name=None, **kwargs):
    ...
    fig, ax = plt.subplots()
    heat = ax.matshow(cov, cmap=plt.get_cmap("Purples"), **kwargs)
