...
class EventMatchedTable:
    def __init__(self, tables):
        ...
        self.index_cols = list(
            self.tables[self.var_labels[0]][self.cut_labels[0]].index.names
        )

    def ToH5(self, output_file):
        if type(output_file) is not h5py.File:
        ...
        for var_label in self.var_labels:
            ...
            for cut_label in self.cut_labels:
                ...
                self.tables[var_label][cut_label] = self.tables[var_label][
                    cut_label
                ].reset_index()

                for col in self.tables[var_label][cut_label].columns:
                    cut_group.create_dataset(
                        col,
    ...
    def Inspect(input_file, only=[]):
        if only and type(only) is not list:
            only = list(only)
        if type(input_file) is not h5py.File:
            ...
            for prop in only:
                ...
                for p in props:
                    print(
                        "%s%s"
                        % (
                            ascii_ex["|--"] if p is not props[-1] else ascii_ex["+--"],
                            p,
                        )
                    )
            ...
            for var_label in var_labels:
                ...
                for cut_label in cut_labels:
                    ...
                    print(
                        "%s%s"
                        % (
                            ascii_ex["+--"] if is_last_cut else ascii_ex["|--"],
                            cut_label,
                        )
                    )
                    for sample in samples:
                        ...
                        print(
                            "%s%s%s%s"
                            % (
                                pad if is_last_cut else ascii_ex["|"],
                                pad,
                                ascii_ex["+--"]
    ...
    def Trim(self, keep_vars, keep_cuts, keep_samples):
        tables = collections.defaultdict(dict)
        for var in keep_vars:
            for cut in keep_cuts:
                tables[var][cut] = pd.concat(
                    [self.tables[var][cut][sample] for sample in keep_samples], axis=1
                )
        return EventMatchedTable(tables)
    ...
    def FromROOT(root_file, index_cols):
        ...
        # read file
        if type(root_file) is not ROOT.TFile:
        ...
        for sample in samples:
            _cut_labels = [
                key.GetName()
                for key in root_file.GetDirectory(sample).GetListOfKeys()
        ...
        for sample in samples:
            for cut_label in cut_labels:
                tree = root_file.Get(os.path.join(sample, cut_label))
                tmp_table[sample][cut_label] = EventMatchedTable.tree_to_table(
                    tree, sample, index_cols
                )
                del tree
                gc.collect()
        root_file.Close()
        ...
        for cut_label in cut_labels:
            ...
            for sample in samples[1:]:
                matched_indices = matched_indices.intersection(
                    tmp_table[sample][cut_label][var_labels[0]].index
                )
            ...
            for var_label in var_labels:
                ...
                for sample in samples:
                    tmp_table[sample][cut_label][var_label] = tmp_table[sample][
                        cut_label
                    ][var_label].loc[matched_indices]

        for var_label in var_labels:
            for cut_label in cut_labels:
                tables[var_label][cut_label] = pd.concat(
                    [tmp_table[sample][cut_label][var_label] for sample in samples],
                    axis=1,
                )
                for sample in samples:
                    tmp_table[sample][cut_label][var_label] = pd.Series()
    ...
    def tree_to_table(tree, label, index):
        ...
        series = {
            var: pd.Series(
                mat[:, var_pos[var]], index=multi_index, name=label, copy=False
            )
            for var in var_pos
        }
        return series
