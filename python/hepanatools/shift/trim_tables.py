...
if type(args.inspect) is list:
    EventMatchedTable.Inspect(args.input, only=args.inspect)

else:
    tables = EventMatchedTable.FromH5(args.input)
    if not args.keep_vars:
        args.keep_vars = tables.var_labels
    if not args.keep_cuts:
        args.keep_cuts = tables.cut_labels
    if not args.keep_samples:
        args.keep_samples = tables.samples

    new_tables = tables.Trim(
