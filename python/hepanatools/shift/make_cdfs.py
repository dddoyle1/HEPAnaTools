...
parser.add_argument('cdf_config',
                    help='JSON file with optimization configuration')
...
parser.add_argument('--report_every', help='Controls the number of fit interations after which to report intermediate results', default=5, type=int)
...
def bins_func(nominal, shifted, xbins, ybins, **kwargs):
    return binopt(*config.ybins_func(nominal, shifted, xbins, ybins),
                  **kwargs)

# progress callback to track fit results throughout the optimization
progress = ProgressTrackerCallback(verbose=args.verbose,
...
cdf = CDF2D(train_nominal,
            train_shifted,
            xbins=config.xbins,
            ybins=config.ybins,
            constraint=config.bounds,
            bins_func=partial(bins_func,
                              callback=progress))
...
if rank == 0:
    ...
    def plot_split(top, bottom, nominal, shifted, bins, cdf):
        hists = {'nominal' : Hist1D(nominal, bins=bins),
        ...
        hists['evt_shifted' ].Draw(top, histtype='step' , color='r', lw=2, label='CDF Shift')
        hists['file_shifted'].Draw(top, histtype='error', color='k', lw=2, label='File Shift', ms=15)

        ratio_evt_shift = Hist1D.Filled(hists['evt_shifted'].n / hists['file_shifted'].n, hists['evt_shifted'].xaxis)
        ratio_nominal   = Hist1D.Filled(hists['nominal'    ].n / hists['file_shifted'].n, hists['nominal'    ].xaxis)
    ...
    ax[0][0], ax[1][0] = plot_split(ax[0][0], ax[1][0],
                                train_nominal,
                                train_shifted,
    ...
    ax[0][1], ax[1][1] = plot_split(ax[0][1], ax[1][1],
                                    test_nominal,
                                    test_shifted,
                                    config.objective_bins,
                                    cdf)
