...
class SKClassifier:
    ...
    def LoadFrom(file_name, classifier):
        with pd.HDFStore(file_name) as f:
            ...
            classifier._dataloader = TrainTestDataLoader(
                train_X=train.drop([classifier._name, "Y"], axis=1),
                train_Y=train["Y"],
    ...
    def Train(self, saveto=None, verbose=True, save_test_train_data=True):
        ...
        self._model.fit(
            self._dataloader._train_X[self._dataloader._feature_vars],
            self._dataloader._train_Y,
        )

        pred_train_y = self._model.predict(
            self._dataloader._train_X[self._dataloader._feature_vars]
        )
        pred_test_y = self._model.predict(
            self._dataloader._test_X[self._dataloader._feature_vars]
        )
        self._prob_train = self._prob_func(
            self._model, self._dataloader._train_X[self._dataloader._feature_vars]
        )
        self._prob_test = self._prob_func(
            self._model, self._dataloader._test_X[self._dataloader._feature_vars]
        )

        ntrain_sig, _ = np.histogram(
            self._prob_train[self._dataloader._train_Y == 1],
            bins=50,
            range=tuple(self._range),
            density=True,
        )
        ntest_sig, _ = np.histogram(
            self._prob_test[self._dataloader._test_Y == 1],
            bins=50,
            range=tuple(self._range),
            density=True,
        )

        ntrain_bkg, _ = np.histogram(
            self._prob_train[self._dataloader._train_Y == 0],
            bins=50,
            range=tuple(self._range),
            density=True,
        )
        ntest_bkg, _ = np.histogram(
            self._prob_test[self._dataloader._test_Y == 0],
            bins=50,
            range=tuple(self._range),
            density=True,
        )
        ...
        training_classification_report["roc_auc"] = roc_auc_score(
            self._dataloader._train_Y, self._prob_train
        )
        ...
        testing_classification_report["roc_auc"] = roc_auc_score(
            self._dataloader._test_Y, self._prob_test
        )
        ...
        if saveto:
            self.SaveTo(saveto, save_test_train_data)
    ...
    def PlotROC(self, plots):
        ...
        plt.plot(
            train_fpr,
            train_tpr,
            label="Train (auc = {:.2})".format(test_auc),
            color="lightsteelblue",
        )
        plt.plot(
            test_fpr,
            test_tpr,
            label="Test  (auc = {:.2})".format(test_auc),
            color="royalblue",
        )
    ...
    def PlotInputVariableLinearCorrelations(self, plots):
        ...
        s_matrix = self._dataloader._train_X[self._dataloader._feature_vars][
            self._dataloader._train_Y == 1
        ].corr()
        b_matrix = self._dataloader._train_X[self._dataloader._feature_vars][
            self._dataloader._train_Y == 0
        ].corr()

        fig, ax = plt.subplots()
        s_heatmap = ax.pcolor(s_matrix, cmap=plt.get_cmap("RdBu"), vmin=-1, vmax=1)
        ...
        ax.set_xticklabels(
            self._dataloader._feature_vars, minor=False, ha="right", rotation=70
        )
        ...
        for (i, j), z in np.ndenumerate(s_matrix):
            ax.text(
                ...
                ha="center",
                va="center",
                color="black" if abs(z) < 0.75 else "white",
            )
        ...
        fig, ax = plt.subplots()
        b_heatmap = ax.pcolor(b_matrix, cmap=plt.get_cmap("RdBu"), vmin=-1, vmax=1)
    ...
    def PlotInputVariableDistributions(self, plots):
        ...
        for col in self._dataloader._feature_vars + self._dataloader._spectator_vars:
            ...
            nbins = 50
            plt.hist(
                self._dataloader._train_X[col][self._dataloader._train_Y == 1],
                bins=nbins,
                range=(minx, maxx),
                alpha=0.7,
                color="b",
            ...
            plt.hist(
                self._dataloader._train_X[col][self._dataloader._train_Y == 0],
                bins=nbins,
                range=(minx, maxx),
                color="r",
                density=True,
            ...
            name = os.path.join(
                plots,
                "{}_var_{}.pdf".format(
                    "feature" if col in self._dataloader._feature_vars else "spectator",
                    col,
                ),
            )
            plt.savefig(name)
...
class BDTClassifier(SKClassifier):
    def __init__(
        ...
        dt = DecisionTreeClassifier(max_depth=max_depth, min_samples_leaf=min_node_size)
        bdt = AdaBoostClassifier(
            dt, algorithm=algorithm, n_estimators=ntrees, learning_rate=ada_boost_beta
        )
...
class GPClassifier(SKClassifier):
    def __init__(self, name, **kwargs):
        ...
        self._prob_func = signal_proba
        self._range = [0, 1]
        super().__init__(name, GaussianProcessClassifier(copy_X_train=False, **kwargs))
...
class ManyClassifiers:
    ...
    def TrainAll(self, *args, **kwargs):
        ...
        for model in self._models:
            ...
            print(model)
            self._training_reports[model], self._testing_reports[model] = self._models[
                model
            ].Train(saveto=os.path.join(self._name, model), *args, **kwargs)
            print()

    def EvaluateAll(
        self, sort_key=lambda report: (report["roc_auc"], report["Signal"]["KS-pvalue"])
    ):

        sorted_testing_reports = dict(
            sorted(
                self._testing_reports.items(),
    ...
    def PlotAllInputVariableDistributions(self):
        for model in self._models:
            self._models[model].PlotInputVariableDistributions(
                os.path.join(self._name, model, "plots")
            )

    def PlotAllInputVariableLinearCorrelations(self):
        for model in self._models:
            self._models[model].PlotInputVariableLinearCorrelations(
                os.path.join(self._name, model, "plots")
            )
