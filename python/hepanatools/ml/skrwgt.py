...
class SKReweighter(SKClassifier):
    ...
    def PlotWeights(self, plots):
        ...
        _, bins, _ = plt.hist(
            self._train_weights[self._dataloader._train_Y == 1],
            bins=100,
            range=(0, max_weight),
            histtype="stepfilled",
            alpha=0.7,
        ...
        plt.hist(
            self._train_weights[self._dataloader._train_Y == 0],
            bins=bins,
            edgecolor="r",
            hatch="/",
            histtype="step",
        ...
        hist, _ = np.histogram(
            self._test_weights[self._dataloader._test_Y == 0], bins=bins, density=True
        )
        bin_widths = np.diff(bins)
        bin_centers = (bins[:-1] + bins[1:]) / 2

        plt.errorbar(
            bin_centers, hist, xerr=bin_widths / 2, fmt=".", c="r", label="B (Test)"
        )

        hist, _ = np.histogram(
            self._test_weights[self._dataloader._test_Y == 1], bins=bins, density=True
        )
        plt.errorbar(
            bin_centers, hist, xerr=bin_widths / 2, fmt=".", c="b", label="S (Test)"
        )
    ...
    def PlotReweightedInputVariableDistributions(self, plots):
        ...
        for col in self._dataloader._feature_vars + self._dataloader._spectator_vars:
            ...
            for icol, sample in enumerate(["training", "testing"]):
                if sample == "training":
                    nom_array = self._dataloader._train_X[col][
                        self._dataloader._train_Y == 0
                    ].values
                    sig_array = self._dataloader._train_X[col][
                        self._dataloader._train_Y == 1
                    ].values
                    weights = self._train_weights[self._dataloader._train_Y == 0]
                elif sample == "testing":
                    nom_array = self._dataloader._test_X[col][
                        self._dataloader._test_Y == 0
                    ].values
                    sig_array = self._dataloader._test_X[col][
                        self._dataloader._test_Y == 1
                    ].values
                    weights = self._test_weights[self._dataloader._test_Y == 0]

                nnom, bins, _ = ax[0][icol].hist(
                    nom_array,
                    range=(minx, maxx),
                    bins=nbins,
                    color="b",
                ...
                bin_centers = (bins[:-1] + bins[1:]) / 2
                ax[0][icol].errorbar(
                    bin_centers, n, xerr=bin_widths / 2, fmt=".", c="k", label="S"
                )
                ...
                leg.get_texts()[0].set_text(
                    r"B             $\chi^2 / dof = %.1f / %d$" % (nom_chi2, nbins)
                )
                leg.get_texts()[1].set_text(
                    r"B (Rwgt) $\chi^2 / dof = %.1f / %d$" % (rwgt_chi2, nbins)
                )
                ...
                def sumw2(n, weights, bins):
                    ...
                    return np.array(
                        [
                            (weights[np.where(bin_idxs == i)] ** 2).sum()
            ...
            plot_path = os.path.join(
                plots, "reweighted_%s_var_%s.pdf" % (var_type, col)
            )
    ...
    def Train(self, *args, **kwargs):
        ...
        for col in self._dataloader._train_X.columns:
            ...
            n_nom_train, bins = np.histogram(
                self._dataloader._train_X[col][self._dataloader._train_Y == 0],
                bins=nbins,
                range=(minx, maxx),
            )
            n_syst_train, _ = np.histogram(
                self._dataloader._train_X[col][self._dataloader._train_Y == 1],
                bins=bins,
            )

            n_nom_rwgt_train, _ = np.histogram(
                self._dataloader._train_X[col][self._dataloader._train_Y == 0],
                bins=bins,
                weights=self._train_weights[self._dataloader._train_Y == 0],
            )

            n_nom_test, bins = np.histogram(
                self._dataloader._test_X[col][self._dataloader._test_Y == 0],
                bins=nbins,
                range=(minx, maxx),
            )
            n_syst_test, _ = np.histogram(
                self._dataloader._test_X[col][self._dataloader._test_Y == 1], bins=bins
            )

            n_nom_rwgt_test, _ = np.histogram(
                self._dataloader._test_X[col][self._dataloader._test_Y == 0],
                bins=bins,
                weights=self._test_weights[self._dataloader._test_Y == 0],
            )
            ...
            chi2_test += np.nan_to_num(
                np.power((n_nom_rwgt_test - n_syst_test), 2) / n_syst_test,
                nan=1,
                posinf=1,
                neginf=1,
            ).sum()
...
class ManyReweighters(ManyClassifiers):
    ...
    def PlotAllReweightedInputVariableDistributions(self):
        for model in self._models:
            self._models[model].PlotReweightedInputVariableDistributions(
